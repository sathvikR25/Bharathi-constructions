  ...
  const fetchUserRole = async (email) => {
    try {
      ...
      const allowedEmails = [
        ...
        "admin@bharathiconstructions.com",
        "md@bharathiconstructions.com",
        "tech@bharathiconstructions.com",
      ...
      if (!isDomainAllowed && !isExplicitlyAllowed) {
        await signOut(auth);
        alert("Security Alert: This email is not authorized for Admin access.");
        ...
        <div className="p-6 md:p-8 md:pb-12 flex justify-between items-center">
          ...
          <button className="md:hidden p-2 bg-white/50 rounded-lg shadow-sm border border-white/80" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
            <div className="space-y-1">
              <span className={`block w-5 h-0.5 bg-[#123645] transition-transform ${mobileMenuOpen ? "rotate-45 translate-y-1.5" : ""}`}></span>
              <span className={`block w-5 h-0.5 bg-[#123645] transition-opacity ${mobileMenuOpen ? "opacity-0" : ""}`}></span>
              <span className={`block w-5 h-0.5 bg-[#123645] transition-transform ${mobileMenuOpen ? "-rotate-45 -translate-y-1.5" : ""}`}></span>
            </div>
          </button>
        </div>
